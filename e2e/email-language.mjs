...
try {
  ...
  for (const [lang, file] of Object.entries(locales)) {
    ...
    for (const width of [390, 1440]) {
      ...
      assert.equal(await page.locator('form button[type="submit"]').isDisabled(), true, 'Signup requires consent even after CAPTCHA')
      ...
      assert.equal(await page.getByRole('checkbox').count(), 0, 'Login must not ask for a consent checkbox')
      await solveCaptcha(page)
      ...
      report.scenarios.push({ language: lang, width, registration: true, signupConsentRequired: true, loginWithoutCheckbox: true, passwordReset: true, emailBindingUI: true, captchaEnforced: true, notifications: kinds.length, dialogWidths: [320,390,768,1440], themes: ['light','dark'], emailOverflow: false })
