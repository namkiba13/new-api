...
try {
  ...
  await request(admin,'/lab/payment','POST',{user_id:sb.id,trade_no:'lab-stripe-1',amount:10})
  ...
  assert.equal((await request(b,'/api/user/topup','POST',{key:redeemKey})).body.success,true)
  ...
  await credit(sb.id,500000,heldKey)
  ...
  const adminPage=await admin.newPage();await adminPage.goto(base+'/system-settings/billing/invite-rewards');await adminPage.locator('#invite-mode').waitFor()
  ...
  console.log(JSON.stringify({passed:true,realLogin:true,realAPIs:true,signedPaymentWebhook:true,signedPartialRefund:true,redemption:true,adminCredits:true,directions:true,replayProtection:true,authorization:true,holdAndReversal:true,languages:7,responsiveWidths:[320,390,768,1024,1440],themes:['light','dark']}))
