...
try {
  ...
  for(const [language,copy] of Object.entries(messages)){
    ...
    for(const width of [320,390,768,1024,1440]){
      ...
      assert.equal(await page.getByText(copy['Only the first eligible credit across all three sources earns rewards.'],{exact:true}).count(),2)
      assert.equal(await page.getByText(copy['After the waiting period, both of you receive rewards'].replace('{{hours}}','24'),{exact:true}).count(),2)
      assert.equal((await page.locator('.invite-rewards-page').innerText()).includes('{{'),false,'No unresolved copy placeholders')
      ...
      assert.equal(await page.getByText(copy['My reward for being invited'],{exact:true}).count(),0,'Incoming rewards are merged into the main summary')
      assert.equal(await page.locator('[data-slot=invite-received]').count(),0)
  ...
  const hc=await account('historyc',hsb.code);const hsc=await state('historyc')
  assert.equal((await credit(hsc.id,500000)).body.success,true)
  ...
  assert.equal((await summary(hb)).pending,2);assert.equal((await summary(hb)).pending_quota,440000,'B combines own welcome award and C referral')
  assert.equal((await summary(hd)).pending,1);assert.equal((await summary(hd)).pending_quota,800000)
  const hap=await ha.newPage();await hap.goto(base+'/invite-rewards');await hap.locator('#invite-link').waitFor()
  const hbp=await hb.newPage();await hbp.goto(base+'/invite-rewards');await hbp.locator('[data-slot=invite-stats]').waitFor()
  const hdp=await hd.newPage();await hdp.goto(base+'/invite-rewards');await hdp.locator('[data-slot=invite-stats]').waitFor()
  assert.equal(await hap.locator('[data-slot=invite-stats] > div').nth(1).locator('dd').first().innerText(),'2')
  assert.equal(await hbp.locator('[data-slot=invite-stats] > div').nth(1).locator('dd').first().innerText(),'2')
  assert.equal(await hbp.locator('[data-slot=invite-stats] > div').nth(1).locator('dd').nth(1).innerText(),'$0.88 pending Credits')
  assert.equal(await hdp.locator('[data-slot=invite-stats] > div').nth(1).locator('dd').first().innerText(),'1')
  assert.equal(await hap.getByRole('button',{name:messages.en['No transferable rewards'],exact:true}).isDisabled(),true,'Pending-only rewards cannot be transferred')
  ...
  assert.equal((await summary(hb)).pending,1);assert.equal((await summary(hb)).pending_quota,40000)
  assert.equal((await summary(hb)).lifetime,400000);assert.equal((await summary(hb)).balance,0)
  await hbp.reload();await hbp.locator('[data-slot=invite-stats]').waitFor()
  assert.equal(await hbp.locator('[data-slot=invite-stats] > div').nth(1).locator('dd').first().innerText(),'1')
  assert.equal(await hbp.locator('[data-slot=invite-stats] > div').nth(5).locator('dd').first().innerText(),'$0.8')
  assert.equal(await hbp.getByRole('button',{name:messages.en['No transferable rewards'],exact:true}).isDisabled(),true,'Welcome reward is already in Credits')
  await hap.reload();await hap.locator('#invite-link').waitFor()
  await hap.getByRole('button',{name:messages.en['Transfer all rewards'],exact:true}).click()
  ...
  const mixed=await summary(hb)
  assert.equal(mixed.qualified,2);assert.equal(mixed.pending,0);assert.equal(mixed.released,2)
  assert.equal(mixed.balance,40000);assert.equal(mixed.lifetime,440000)
  await hbp.reload();await hbp.locator('[data-slot=invite-stats]').waitFor()
  assert.equal(await hbp.locator('[data-slot=invite-stats] > div').nth(4).locator('dd').first().innerText(),'$0.08')
  assert.equal(await hbp.locator('[data-slot=invite-stats] > div').nth(5).locator('dd').first().innerText(),'$0.88')
  await hbp.getByRole('button',{name:messages.en['Transfer all rewards'],exact:true}).click()
  await hbp.getByText('Rewards added to credits',{exact:true}).waitFor()
  assert.equal((await state('historyb')).quota,5440000,'Mixed-role B transfers only $0.08, never its $0.80 welcome reward again')
  assert.equal((await state('historyc')).quota,540000)
  assert.equal((await summary(hb)).balance,0);assert.equal((await summary(hb)).lifetime,440000)
  assert.equal((await summary(hc)).balance,0);assert.equal((await summary(hc)).lifetime,40000)
  ...
  if(process.env.SCREENSHOT_DIR) await hbp.locator('[data-slot=invite-stats]').screenshot({path:`${process.env.SCREENSHOT_DIR}/94api-invite-combined-user.png`})
  ...
  console.log(JSON.stringify({passed:true,firstOnly:true,realLogin:true,realAPIs:true,signedPaymentWebhook:true,signedPartialRefund:true,redemption:true,adminCredits:true,directions:true,replayProtection:true,authorization:true,holdAndReversal:true,languages:7,responsiveWidths:[320,390,768,1024,1440],themes:['light','dark'],themeButtonAndPersistence:true,statIcons:6,combinedPendingUI:true,adminHistory:true,journalIsolation:true,rateUpdate8to10:true,balanceSnapshots:true,userSummaryOnly:true,pendingTransferRejected:true,referralWalletTransfer:true,mixedRoleTotals:true,noDoubleWalletCredit:true}))
