...
try {
  ...
  for(const [language,copy] of Object.entries(messages)){
    ...
    for(const width of [320,390,768,1024,1440]){
      ...
      assert.equal(await page.locator('[data-slot=invite-user-history]').count(),0,'No detailed history block on the User page')
      assert.equal(await page.getByText(copy['Rewards from my referrals'],{exact:true}).count(),1,'Referral summary appears once')
      assert.equal(await page.getByText(copy['My reward for being invited'],{exact:true}).count(),1,'Incoming summary appears once')
  ...
  assert.equal(await hap.getByRole('button',{name:'No reward balance yet',exact:true}).isDisabled(),true,'Pending-only rewards cannot be transferred')
  assert.equal((await request(ha,'/api/user/invite-rewards/transfer','POST',{}, {'Idempotency-Key':randomUUID()})).body.success,false,'Backend rejects pending-only transfer')
  ...
  assert.equal((await state('historya')).quota,400000,'Released reward enters the inviter wallet')
  assert.equal((await state('historyb')).quota,5400000,'Inviter transfer does not change the friend wallet')
  assert.equal((await state('historyd')).quota,10000000,'Pending friend reward is not spendable')
  ...
  assert.equal(await hbp.locator('[data-slot=invite-user-history]').count(),0)
  if(process.env.SCREENSHOT_DIR) await hbp.locator('[data-slot=invite-received]').screenshot({path:`${process.env.SCREENSHOT_DIR}/94api-invite-summary-user.png`})
