...
export function UserOverviewDashboard() {
  ...
  const timeRange = useMemo(() => {
    ...
    return { start_timestamp: end - 86400, end_timestamp: end }
  }, [])
  ...
  const recentUsage = (usageQuery.data?.data ?? []).reduce(
    (total, item) => total + (Number(item.quota) || 0),
    0
  )
  let recentUsageDisplay = t('Loading')
  if (usageQuery.isError || usageQuery.data?.success === false) {
    recentUsageDisplay = t('Failed to load')
  } else if (usageQuery.isSuccess) {
    recentUsageDisplay = formatQuota(recentUsage)
  }
  return (
    <div className='apimore-user-overview flex flex-col gap-4 sm:gap-5'>
      <section className='bg-card relative overflow-hidden border shadow-xs'>
        ...
        <div className='flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between sm:px-5'>
          <div className='flex min-w-0 items-center gap-3'>
            <span className='flex size-8 shrink-0 items-center justify-center rounded-md bg-lime-300 text-black'>
          ...
          <Link
            to='/keys'
            className='overview-create-key group border-foreground bg-foreground text-background inline-flex h-10 w-full shrink-0 items-center justify-between gap-3 rounded-[2px] border px-4 text-sm font-semibold transition-[background-color,color,border-color,transform] outline-none hover:border-lime-300 hover:bg-lime-300 hover:text-black focus-visible:ring-2 focus-visible:ring-lime-400 focus-visible:ring-offset-2 active:translate-y-px sm:w-auto sm:min-w-48'
          >
      ...
      <section className='bg-card overflow-hidden border shadow-xs'>
        <header className='flex flex-wrap items-center justify-between gap-3 border-b px-4 py-4 sm:px-5'>
          <div>
            <h3 className='text-sm font-semibold'>{t('Credits and usage')}</h3>
            <p className='text-muted-foreground mt-1 text-xs sm:text-sm'>
              {t('Monitor credits, usage, and request volume')}
            </p>
          </div>
          <Button
            className='overview-secondary !rounded-[3px]'
            size='sm'
        ...
        <div className='flex flex-wrap items-center justify-between gap-2 border-y px-4 py-3 text-xs sm:px-5'>
          ...
          <Button
            className='overview-link !rounded-[3px]'
            variant='ghost'
            size='sm'
            render={<Link to='/wallet' />}
          >
...
function UsageCell(props: {
  ...
  return (
    <dl className='min-w-0 px-4 py-4 sm:px-5 sm:py-5'>
