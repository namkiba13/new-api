...
func SendEmailVerification(c *gin.Context) {
	...
	err := service.SendLocalizedEmail(i18n.ParseAcceptLanguage(c.GetHeader("Accept-Language")), i18n.EmailVerification, email,
		map[string]any{"Code": code, "Minutes": common.VerificationValidMinutes})
	if err != nil {
...
func SendPasswordResetEmail(c *gin.Context) {
	...
	if _, err := model.GetUniqueUserByEmail(email); err == nil {
		...
		link := fmt.Sprintf("%s/user/reset?%s", system_setting.ServerAddress, url.Values{"email": {email}, "token": {code}}.Encode())
		err := service.SendLocalizedEmail(i18n.ParseAcceptLanguage(c.GetHeader("Accept-Language")), i18n.EmailPasswordReset, email,
			map[string]any{"Link": link, "Minutes": common.VerificationValidMinutes})
		if err != nil {
